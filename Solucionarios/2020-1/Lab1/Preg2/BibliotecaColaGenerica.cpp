...
void creaCola(void *&cola, void *(*funcLeer)(ifstream &)) {
    void **colaAux, *dato;
    // Abrir el archivo
    ifstream arch("ClientesSeg.csv", ios::in);
    if (!arch) {
...
void encola(void *cola, void *dato) {
    ...
    regNuevo = new void*[2];
    regNuevo[DATO] = dato;
    regNuevo[SIG] = nullptr;
    // Colocar el registro en la cola
    colaAux = (void**)cola;
    if (colaVacia(cola)) {
        colaAux[INICIO] = regNuevo;
    } else {
        regAnterior = (void**)(colaAux[FIN]);
        regAnterior[SIG] = regNuevo;
    }
    // Reemplazar el final de la cola
    colaAux[FIN] = regNuevo;
}
...
void *desencola(void *cola) {
    ...
    if (colaVacia(cola)) {
        ...
        colaAux = (void**)cola;
        regInicio = (void**)(colaAux[INICIO]);
        dato = regInicio[DATO];
        // Reemplazar el inicio de la cola
        colaAux[INICIO] = regInicio[SIG];
        delete regInicio;
...
void imprimir(void *cola, void (*funcImprimir)(ofstream &, void *)) {
    ...
    if (colaVacia(cola)) {
        ...
        colaAux = (void**)cola;
        reg = (void**)(colaAux[INICIO]);
        while (reg != nullptr) {
            funcImprimir(arch, reg[DATO]);
            reg = (void**)(reg[SIG]);
        }
    }
}
...
bool colaVacia(void *cola) {
    ...
    return colaAux[INICIO] == nullptr || colaAux[FIN] == nullptr;
}
