...
void Procesa::consolida() {
    ifstream archSancionados = abrirArchivo("Sancionados.csv");
    ofstream archReporte = crearArchivo("Consolidado.txt");
    int licencia, puntos, meses;
    double multas;
    char* nombre;
    while (true) {
        archSancionados >> licencia;
        if (archSancionados.eof()) break;
        // Inicializar datos
        multas = puntos = meses = 0;
        nombre = nullptr;
        // Buscar las incidencias que coincidan con la licencia
        for (int i = 0; i < cantidad; i++) {
            if (licencia == lregistro[i].GetLicencia()) {
                if (nombre == nullptr) nombre = lregistro[i].GetNombre();
                lregistro[i].actualizarEstadisticas(multas, puntos, meses);
            }
        }
        // Imprimir reporte del conductor
        imprimirConductor(archReporte, licencia, nombre, multas, puntos, meses);
    }
}

void Procesa::imprimirConductor(ofstream& arch, int licencia, char* nombre,
        double multas, int puntos, int meses) {
    arch << "Conductor: " << nombre << endl;
    arch << "Licencia No.: " << licencia << endl;
    imprimirLinea(arch, '-', MAX_LINEA_PREG2);
    arch << "Monto de multas: " << multas << endl;
    arch << "Puntos perdidos: " << puntos << endl;
    arch << "Meses sancionados: " << meses << endl << endl;
    delete nombre;
}
