...
void Registro::actualizarEstadisticas(double& multas, int& puntos, int& meses) {
    if (lleve.GetCodigo() != 0) {
        multas += lleve.GetMulta() * (1 - lleve.GetDescuento());
    }
    if (lgrave.GetCodigo() != 0) {
        multas += lgrave.GetMulta() * (1 - lgrave.GetDescuento());
        puntos += lgrave.GetPuntos();
    }
    if (lmgrave.GetCodigo() != 0) {
        multas += lmgrave.GetMulta();
        puntos += lmgrave.GetPuntos();
        meses += lmgrave.GetMeses();
    }
}
