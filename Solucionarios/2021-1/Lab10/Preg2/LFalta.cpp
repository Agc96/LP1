...
void LFalta::unir(class LFalta& lfalta) {
    this->lfin->sig = lfalta.lini;
    this->lfin = lfalta.lfin;
    lfalta.lini = lfalta.lfin = nullptr;
}
...
void LFalta::imprimir(const char* nomArch) {
    ...
    // Imprimir cabecera
    arch << "Fecha" << setw(15) << "Licencia" << setw(9) << "Placa";
    arch << setw(15) << "Infraccion" << setw(11) << "Monto";
    arch << setw(14) << "Descuento" << setw(8) << "Puntos";
    arch << setw(15) << "Meses susp." << endl;
    imprimirLinea(arch, '-', 100);
    // Imprimir listado de faltas cometidas
...
void LFalta::imprimirLinea(ofstream& arch, char c, int num) {
    for (int i = 0; i < num; i++) {
        arch.put(c);
    }
    arch << endl;
